...
def get_active_games(player_id):
    ...
    if response.status_code == 200:
        data = response.json()
        print(f"API Response for player ID {player_id}: {data}")  # Debug print
...
def extract_opponent_info(game, player_id):
    ...
    # Debug print to inspect opponent data
    print(f"Opponent Data: {opponent}")
    
    # Ensure the 'rating' field exists and get the rating
...
def rank_number_to_label(rating):
    # Rank to label conversion based on rating
    if rating >= 2600:
        return "9d"
        ...
        return "1d"
    elif 1750 <= rating < 1800:
        return "1k"
    elif 1700 <= rating < 1750:
        return "2k"
        ...
        return "25k"  # All ratings below 600 are considered 25k
